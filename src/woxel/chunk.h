...
class WoxelChunk
{
  private:
    ...
    std::int64_t linear_index(std::int32_t i_theta, std::int32_t i_z, std::int32_t i_r) const
    {
        ...
        if (i_theta < 0 || i_theta >= m_dims.Ctheta || i_z < 0 || i_z >= m_dims.Cz || i_r < 0 || i_r >= m_dims.Cr)
        {
  ...
  public:
    WoxelChunk(const WoxelSpaceSpec& spec, const ChunkDims& dims, const ChunkAddress& addr) :
        m_spec(spec), m_dims(dims), m_addr(addr), m_cells(static_cast<std::size_t>(dims.cell_count()), 0)
    {
    }

    const ChunkDims& dims() const
    {
        return m_dims;
    }
    const ChunkAddress& address() const
    {
        return m_addr;
    }
