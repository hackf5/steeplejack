...
inline size_t total_bytes(const std::ranges::contiguous_range auto& range) {
...
void copy_to(void* dst, TIter begin, TIter end) {
    ...
    static_assert(std::is_standard_layout_v<std::decay_t<decltype(*begin)>>,
                  "TIter must be an iterator to a standard layout type");
    std::memcpy(dst, to_void_address(begin), total_bytes(begin, end));
}

inline void copy_to(void* dst, const std::ranges::contiguous_range auto& range) {
